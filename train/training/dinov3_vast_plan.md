...
- 구현 범위는 실험 전용이다. `Mirip` 운영 추론과 직접 호환시키지 않고, `train/output_models` 아래에서 checkpoint, logs, anchors를 독립 산출물로 관리한다.
...
- 산출물:
  - checkpoints: `train/output_models/checkpoints/dinov3_vitl16/`
  - logs: `train/output_models/logs/dinov3_vitl16_{smoke,full}.json`
  - anchors: `train/output_models/anchors/anchors.pt`
- full 후처리 비교 결과는 `train/output_models/logs/dinov3_vit7b16_postprocess_registry.json`에 누적한다.
...
9. `Mirip_v2/train/data`, `Mirip_v2/train/training/data`, `Mirip_v2/train/output_models/logs`를 rsync 업로드
10. remote bootstrap: Python env, `transformers`/`wandb` 등 설치
11. remote `validate-upload` 실행
12. smoke run
13. smoke 통과 시 full run
14. `train/output_models/checkpoints`, `train/output_models/logs`, `train/output_models/anchors`를 local로 회수
15. 인스턴스 destroy
