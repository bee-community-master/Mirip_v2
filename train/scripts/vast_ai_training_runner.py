...
def _build_smoke_command(remote_root: str) -> str:
    ...
    return _bash_command(
        [
            "set -euo pipefail",
            f"cd {shlex.quote(remote_root)}",
            f"{python_bin} {TRAINING_DIR}/train_dinov3.py --pairs-train training/data/pairs_train.csv --pairs-val training/data/pairs_val.csv --image-root data --output-dir {checkpoint_dir} --model-name {shlex.quote(TRAIN_MODEL_NAME)} --backbone-dtype auto --epochs 1 --batch-size {TRAIN_BATCH_SIZE} --gradient-accumulation-steps {TRAIN_GRADIENT_ACCUMULATION_STEPS} --learning-rate 3e-5 --backbone-learning-rate-scale 0.2 --unfreeze-last-n-layers 2 --patience 3 --early-stopping-metric anchor_tier_accuracy --num-workers {TRAIN_NUM_WORKERS} --prefetch-factor {TRAIN_PREFETCH_FACTOR} --report {REPORTS_REL_DIR}/{TRAIN_MODEL_SLUG}_smoke_train.json",
...
def _build_full_command(remote_root: str) -> str:
    ...
    smoke_report = f"{REPORTS_REL_DIR}/{TRAIN_MODEL_SLUG}_smoke.json"
    ...
    return _bash_command(
        [
            "set -euo pipefail",
            f"cd {shlex.quote(remote_root)}",
            f"{python_bin} {TRAINING_DIR}/train_dinov3.py --pairs-train training/data/pairs_train.csv --pairs-val training/data/pairs_val.csv --image-root data --output-dir {checkpoint_dir} --model-name {shlex.quote(TRAIN_MODEL_NAME)} --backbone-dtype auto --resume-from {smoke_checkpoint} --epochs 50 --batch-size {TRAIN_BATCH_SIZE} --gradient-accumulation-steps {TRAIN_GRADIENT_ACCUMULATION_STEPS} --learning-rate 3e-5 --backbone-learning-rate-scale 0.2 --unfreeze-last-n-layers 2 --patience 3 --early-stopping-metric anchor_tier_accuracy --num-workers {TRAIN_NUM_WORKERS} --prefetch-factor {TRAIN_PREFETCH_FACTOR} --report {TRAIN_FULL_TRAIN_REPORT} --postprocess-metadata-train training/data/metadata_train.csv --postprocess-metadata-eval training/data/metadata_val.csv --postprocess-anchors-output {TRAIN_FULL_CANDIDATE_ANCHORS} --postprocess-report {TRAIN_FULL_CANDIDATE_REPORT} --postprocess-registry {TRAIN_FULL_REGISTRY} --postprocess-best-checkpoint {smoke_checkpoint} --postprocess-best-report {smoke_report}",
